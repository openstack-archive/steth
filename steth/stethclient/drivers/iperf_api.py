...
class CheckIperf(Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('iperf_client_ip', default='bad')
    ...
    def take_action(self, parsed_args):
        ...
        if utils.is_ip(parsed_args.iperf_client_ip):
            Logger.log_fail('IP address not legal')
            sys.exit()
        # check iperf client ip exist
        res = client.validate_ip(parsed_args.iperf_client_ip)
        if res['code'] == 1:
        ...
        res = client.start_iperf_client(protocol=parsed_args.client_protocol,
                                        host=parsed_args.iperf_client_ip,
